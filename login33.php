...
if($roles=="admin")
{
echo "<script>window.open('adminhomepage.php?firstname=$firstname','_self')</script>";
die();
}
else if($roles=="doctor")
{
echo "<script>window.open('doctorhomepage.php?firstname=$firstname','_self')</script>";
die();
}
else if ($roles=="laboratorist") {
  echo "<script>window.open('laboratoristhomepage.php?firstname=$firstname','_self')</script>";
die();
}
}}
else{
...
<FORM class="contact_form" action="login33.php" method="post" name="contact_form"  onsubmit="return(validate())";>     
<ul>
    <li>
